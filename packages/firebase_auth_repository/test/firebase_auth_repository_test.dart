...
void main() {
  ...
  group('AuthenticationRepository', () {
    ...
    test('creates FirebaseAuth instance internally when not injected', () {
      expect(FirebaseAuthRepository.new, isNot(throwsException));
    });
    ...
    group('signUp', () {
      ...
      test('calls createUserWithEmailAndPassword', () async {
        await authenticationRepository.signUpWithEmailAndPassword(email: email, password: password);
        verify(
          () => firebaseAuth.createUserWithEmailAndPassword(
            email: email,
            password: password,
      ...
      test('succeeds when createUserWithEmailAndPassword succeeds', () async {
        expect(
          authenticationRepository.signUpWithEmailAndPassword(email: email, password: password),
          completes,
        );
      });
    ...
    group('user', () {
      ...
      test('emits User when firebase user is not null', () async {
        ...
        verify(
          () => cache.write(
            key: FirebaseAuthRepository.userCacheKey,
            value: user,
          ),
        ).called(1);
      });
    ...
    group('currentUser', () {
      test('returns User.empty when cached user is null', () {
        when(
          () => cache.read(key: FirebaseAuthRepository.userCacheKey),
        ).thenReturn(null);
        expect(
      ...
      test('returns User when cached user is not null', () async {
        when(
          () => cache.read<User>(key: FirebaseAuthRepository.userCacheKey),
        ).thenReturn(user);
