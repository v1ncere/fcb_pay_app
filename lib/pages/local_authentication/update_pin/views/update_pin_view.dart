...
import 'package:fcb_pay_app/pages/local_authentication/update_pin/widgets/widgets.dart';
...
class UpdatePinView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text(AppString.updatePin, style: TextStyle(color: Colors.black)),
        leading: BackButton(
        ...
        listener: (context, state) {
          if (state.status.isCurrentEquals) {
            showDialog(
              ...
              builder: (context) => AlertDialog(
                ...
                title: const Center(child: Text(AppString.currentPinAccepted, textAlign: TextAlign.center)),
          ...
          if (state.status.isCurrentUnequals) {
            showDialog(
              ...
              builder: (context) => AlertDialog(
                shape: const RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(15))),
                title: const Center(child: Text(AppString.currentPinRejected, textAlign: TextAlign.center)),
                actionsAlignment: MainAxisAlignment.center,
                actions: [
                  TextButton(
                    onPressed: () => Navigator.pop(context),
          ...
          if (state.status.isUpdateEquals) {
            showDialog(
              ...
              builder: (context) => AlertDialog(
                shape: const RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(15))),
                title: const Center(child: Text(AppString.updatePinSuccess, textAlign: TextAlign.center)),
                actionsAlignment: MainAxisAlignment.center,
                actions: [
                  TextButton(
